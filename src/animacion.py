...
class AnimacionGeneral:
    ...
    def configurar_desplazamiento_etiqueta(self, componente_etiqueta, textos_dict, longitud_maxima):
        ...
        if hasattr(self, "id_marcador_tiempo") and self.id_marcador_tiempo:
            componente_etiqueta.after_cancel(self.id_marcador_tiempo)
            self.id_marcador_tiempo = None
        self.desplazamiento_activo = {}
        self.posicion_desplazamiento = {}
        self.direccion_desplazamiento = {}
        ...
        if any(self.desplazamiento_activo.values()):
            ...
            self.iniciar_desplazamiento_etiqueta(componente_etiqueta, min_longitud_letra)

    # Método para iniciar desplazamiento de textos largos (llama a animar)
    def iniciar_desplazamiento_etiqueta(self, componente_etiqueta, longitud_maxima):
        # Cancelar cualquier animación anterior
        if hasattr(self, "id_marcador_tiempo") and self.id_marcador_tiempo:
            componente_etiqueta.after_cancel(self.id_marcador_tiempo)
            self.id_marcador_tiempo = None
        ...
        if any(self.desplazamiento_activo.values()):
            self.animar_desplazamiento_etiqueta(componente_etiqueta, longitud_maxima)

    # Método para detener la animación
    def detener_desplazamiento_etiqueta(self, componente_etiqueta):
        if hasattr(self, "id_marcador_tiempo") and self.id_marcador_tiempo:
            componente_etiqueta.after_cancel(self.id_marcador_tiempo)
            self.id_marcador_tiempo = None
    ...
    def animar_desplazamiento_etiqueta(
        self, componente_etiqueta, longitud_maxima, intervalo=125, reproduciendo=True
    ):
        ...
        if not reproduciendo:
            self.id_marcador_tiempo = componente_etiqueta.after(
                400,
                lambda: self.animar_desplazamiento_etiqueta(
                    componente_etiqueta, longitud_maxima, intervalo, reproduciendo
                ),
            )
            return
